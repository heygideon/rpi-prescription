...
export default function OrderPrepare() {
  ...
  const { device } = useLockerConnection({
  ...
  const openLockerMutation = useMutation({
    mutationFn: async () => {
      if (!device || !selectedLocker) throw "";
      try {
        const { code } = await utils.client.admin.orders.genCode.query();
        await BleClient.write(
          device.deviceId,
          "A07498CA-AD5B-474E-940D-16F1FBE7E8CD",
          "478264A7-AF0A-4E22-A666-CC8E8B065FF1",
          textToDataView(`id:${params.id!};l:${selectedLocker};code:${code}`),
        );
      } catch (e) {
        console.error(e);
        throw e;
  ...
  const { longPressProps } = useLongPress({
    threshold: 600,
    isDisabled: lockerOpen,
    onLongPress: () => {
      openLockerMutation.mutate();
    },
  });
  ...
  return (
    <div>
      <div className="mx-auto max-w-6xl px-8 pt-6">
        <div className="flex h-12 items-center gap-1.5 overflow-clip rounded-lg border border-gray-300 bg-gray-50 px-4 shadow-sm">
          {device ? (
            <>
      ...
      <div className="divide-y divide-gray-300">
        ...
        <section>
          <div className="mx-auto max-w-6xl px-8 py-6">
            ...
            >
              <p className="pt-4 text-xl font-semibold tracking-tight">
                avocet
              </p>
